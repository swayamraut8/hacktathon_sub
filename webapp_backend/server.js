...
const uri = 'mongodb+srv://swayam88:1234@cluster0.wnwpthu.mongodb.net/product?retryWrites=true&w=majority'; // Replace with your MongoDB Atlas connection string
  ...
  try {
    ...
    if (product.ProductOwner === 'supermarket') {
      ...
      if (result.modifiedCount > 0) {
        return res.json({ success: true, message: 'Product purchased successfully', product });
      } else {
        return res.status(400).json({ error: 'Product update failed' });
      }
    } else {
      return res.json({ success: true, message: 'Product already owned', product });
    }
  } catch (error) {
    console.error('Error processing request:', error);
    res.status(500).json({ error: 'Internal Server Error' });
...
app.post('/returnProduct', async (req, res) => {
  ...
  try {
    ...
    if (result.modifiedCount > 0) {
      return res.json({ success: true, message: 'Product returned to supermarket' });
    } else {
      return res.status(404).json({ error: 'Product not found or not owned by swayam_raut' });
    }
  } catch (error) {
    console.error('Error processing request:', error);
    res.status(500).json({ error: 'Internal Server Error' });
...
app.get('/purchases', async (req, res) => {
  try {
    ...
    const products = await collection.find({ ProductOwner: 'swayam_raut' }).toArray();
